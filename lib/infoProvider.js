...
const getDistrictCases = async (name) => {
  ...
  covidCases.forEach((state) => {
    state.districtData.forEach((districtInfo) => {
      if (districtInfo.name === name) districtData = districtInfo;
    });
  });
  return districtData;
  // const state = Object.keys(covidCases).find((state) => {
  //   return Object.keys(covidCases[state].districtData).some(
  //     (districtName) => districtName.toLowerCase() === name.toLowerCase()
  //   );
  // });
